/*
    ...
    OpenGL loader generated by glad 0.1.24a0 on Fri Jun  8 15:38:15 2018.
...
static int get_exts(void) {
    ...
    if(max_loaded_major < 3) {
        ...
        if (num_exts_i > 0) {
            exts_i = (char **)lens::realloc((void *)exts_i, (size_t)num_exts_i * (sizeof *exts_i));
        }
        ...
        for(index = 0; index < (unsigned)num_exts_i; index++) {
            ...
            size_t len = lens::strlen(gl_str_tmp);

            char *local_str = (char*)lens::malloc((len+1) * sizeof(char));
            if(local_str != NULL) {
				/*
#if _MSC_VER >= 1400
...
#endif
				*/
				lens::strlcpy(local_str, gl_str_tmp, len + 1);
            }
...
static void free_exts(void) {
    if (exts_i != NULL) {
        ...
        for(index = 0; index < num_exts_i; index++) {
			lens::free((char *)exts_i[index]);
        }
        lens::free((void *)exts_i);
        exts_i = NULL;
    }
}
...
static int has_ext(const char *ext) {
    ...
    if(max_loaded_major < 3) {
        ...
        while(1) {
            loc = lens::strstr(extensions, ext);
            if(loc == NULL) {
                return 0;
            }

            terminator = loc + lens::strlen(ext);
        ...
        for(index = 0; index < num_exts_i; index++) {
            ...
            if(exts_i[index] != NULL && lens::strcmp(e, ext)) {
                return 1;
            }
        }
    }
...
static void find_coreGL(void) {
    ...
    for (i = 0;  prefixes[i];  i++) {
        const size_t length = lens::strlen(prefixes[i]);
        //if (strncmp(version, prefixes[i], length) == 0) {
		if (lens::strncmp(version, prefixes[i], length)) {
            version += length;
            break;
        }
    }
	...
	/*
	// No scanf implementation, assume we have 3.3
// PR #18
#ifdef _MSC_VER
